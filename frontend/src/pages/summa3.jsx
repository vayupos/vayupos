...
function POS() {
  ...
  const createOrder = async (paymentMethod) => {
    ...
    try {
      ...
      // Build order payload - ✅ FIXED: Changed 'items' to 'order_items'
      const orderPayload = {
        customer_id: selectedCustomerId || null,
        order_items: orderItems,  // ✅ FIXED: Changed from 'items' to 'order_items'
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      // ✅ FIXED: Changed 'items' to 'order_items'
      const draftPayload = {
        customer_id: selectedCustomerId || null,
        order_items: orderItems,  // ✅ FIXED: Changed from 'items' to 'order_items'
        subtotal: parseFloat(orderSubtotal.toFixed(2)),
    ...
    const invoiceHTML = `
      ...
      <head>
        ...
        <style>
          ...
          .grand-total { font-size: 1.2em; font-weight: bold; margin-top: 10px; padding-top: 10px; border-top: 2px solid #000; }@media print {
        body { padding: 0; }
        button { display: none; }
      }
    </style>
